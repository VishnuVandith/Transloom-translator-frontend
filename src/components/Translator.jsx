...
function Translator({ setReqAccessOpen, JWT }) {
    ...
    if (supportedCombinations.includes(combination)) {
      ...
      try {
        ...
        const response = await userAxiosInstence.get(
          `/translation/translate/${inputLanguage}/${outputLanguage}?text=${inputText}`
        );
        if (response.data) {
  ...
  // Check if 'isLogin' exists and log it to the console
